...
- `voicevox/`: Build Text / Build Project 用の変換設定
  - `synthesis_defaults.example.json`: 共有用テンプレート（`--synthesis-defaults` で明示指定して利用）
  - `synthesis_defaults.json`: 既定の synthesis defaults（`--synthesis-defaults` 未指定時に使用、gitignore）
