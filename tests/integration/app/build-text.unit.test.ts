...
test("splitIntoSentences falls back to max-char split when no boundary exists", () => {
  const actual = splitIntoSentences("abcdefghijklmnop", {
    maxCharsPerSentence: 6,
  });
...
test("splitIntoSentences avoids trailing tiny fragments when sentence barely exceeds limit", () => {
  ...
  assert.equal(
    actual.some((sentence) => sentence === "。" || sentence === "す。"),
    false,
  );
...
test("decidePauseLengthMs uses punctuation strength and continuation context", () => {
  ...
  assert.equal(
    decidePauseLengthMs("そして進める、", { isTerminalInSourceLine: false }),
    150,
  );
  assert.equal(
    decidePauseLengthMs("abcdef", { isTerminalInSourceLine: false }),
    140,
  );
});
...
test("inferReadingFromSurface infers katakana and uppercase acronym readings", () => {
  ...
  assert.equal(
    inferReadingFromSurface("ECMASCRIPT"),
    "イーシーエムエーエスシーアールアイピーティー",
  );
...
test("dictionary candidate extraction infers readings and excludes low-signal tokens", () => {
  ...
  assert.equal(
    candidates.some((item) => item.surface === "any"),
    false,
  );
});
...
test("priority balances source reliability and occurrences", () => {
  ...
  assert.equal(
    candidates.find((item) => item.surface === "FFI")?.priority,
    "LOW",
  );
  assert.equal(
    candidates.find((item) => item.surface === "検証")?.priority,
    "MEDIUM",
  );
  assert.equal(
    candidates.find((item) => item.surface === "ユースケース")?.priority,
    "HIGH",
  );
});
