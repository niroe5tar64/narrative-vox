...
async function createMockFfmpegScript(params?: {
	...
	const tempRoot = await mkdtemp(
		path.join(os.tmpdir(), "narrative-vox-ffmpeg-test-"),
	);
...
test("build-audio uses stage5 query directly when present", async () => {
	...
	let receivedSamplingRate: unknown;
	...
	await withMockVoicevoxServer(
		...
		async (voicevoxApiUrl) => {
			...
			assert.equal(
				manifest.output.merged_wav_path,
				path.join("audio", "E01.wav"),
			);
			assert.equal(
				manifest.utterances[0]?.wav_path,
				path.join("audio", "E01.wav"),
			);
	...
	await withMockVoicevoxServer(
		...
		async (voicevoxApiUrl) => {
			...
			const firstManifest = JSON.parse(
				await readFile(first.manifestPath, "utf-8"),
			) as BuildAudioManifest;
			...
			const secondManifest = JSON.parse(
				await readFile(second.manifestPath, "utf-8"),
			) as BuildAudioManifest;
...
test("build-audio outputs one merged wav for multiple utterances", async () => {
	...
	await withMockVoicevoxServer(
		...
		async (voicevoxApiUrl) => {
			...
			assert.equal(
				manifest.utterances[1]?.wav_path,
				path.join("audio", "E01.wav"),
			);
		},
	);
...
test("build-audio converts merged wav to mp3 when ffmpeg succeeds", async () => {
	...
	await withMockVoicevoxServer(
		...
		async (voicevoxApiUrl) => {
			...
			assert.equal(
				manifest.output.compressed_audio_path,
				path.join("audio", "E01.mp3"),
			);
...
test("build-audio partial failure: first utterance fails at audio_query, second succeeds", async () => {
	...
	await withMockVoicevoxServer(
		...
		async (voicevoxApiUrl) => {
			...
			assert.equal(result.failures[0]?.statusCode, 400);
			assert.equal(
				typeof result.mergedWavPath,
				"string",
				"merged WAV should still be written for successful segments",
			);
	...
	await withMockVoicevoxServer(
		...
		async (voicevoxApiUrl) => {
			...
			assert.equal(
				typeof result.mergedWavPath,
				"string",
				"merged WAV should contain the one successful segment",
			);
...
test("build-audio all utterances fail: no merged WAV is written", async () => {
	...
	await withMockVoicevoxServer(
		...
		async (voicevoxApiUrl) => {
			...
			assert.equal(
				result.mergedWavPath,
				undefined,
				"no merged WAV should be written when all utterances fail",
			);
...
test("build-audio records compression failure when ffmpeg exits with error", async () => {
	...
	await withMockVoicevoxServer(
		...
		async (voicevoxApiUrl) => {
			...
			assert.equal(
				result.compression.error?.includes("ffmpeg conversion failed"),
				true,
			);
			...
			assert.equal(
				manifest.compression.error?.includes("mocked ffmpeg failure"),
				true,
			);
