...
const defaultBuildTextConfigPath = path.resolve(
	"configs/voice/voicevox/build-text-config.json",
);
...
type BuildTextInput = Omit<
	Parameters<typeof buildTextBase>[0],
	"buildTextConfigPath"
> & {
...
function buildText(options: BuildTextInput) {
	return buildTextBase({
		...options,
		buildTextConfigPath:
			options.buildTextConfigPath ?? defaultBuildTextConfigPath,
	});
}
...
function createAudioQueryHandler(options?: {
	...
	responseFactory?: (
		context: AudioQueryHandlerContext,
	) => Record<string, unknown>;
	maxCalls?: number;
...
test("build-text -> build-project pipeline works with sample script", async () => {
	...
	const textJson = JSON.parse(
		await readFile(buildTextResult.voicevoxTextJsonPath, "utf-8"),
	...
	await withMockVoicevoxServer(
		...
		async (voicevoxApiUrl) => {
			...
			const projectJson = JSON.parse(
				await readFile(projectResult.importJsonPath, "utf-8"),
			) as VoicevoxProjectJsonTest;
			assert.equal(
				projectJson.talk.audioKeys.length,
				textJson.utterances.length,
			);
			const firstAudioItem =
				projectJson.talk.audioItems[projectJson.talk.audioKeys[0]];
...
test("build-text applies reading dictionary entries to generated utterances", async () => {
	...
	assert.equal(
		utteranceTexts.some((text) => text.includes("リスクリプト")),
		true,
	);
	assert.equal(
		utteranceTexts.some((text) => text.includes("ReScript")),
		false,
	);
});
...
test("build-text keeps ruby reading precedence over reading dictionary", async () => {
	...
	await writeFile(
		scriptPath,
		["1. 導入", "{ReScript|リ・スクリプト}で検証します。"].join("\n"),
		"utf-8",
	);
	...
	const utteranceTexts = textJson.utterances.map((utterance) => utterance.text);
	assert.equal(
		utteranceTexts.some((text) => text.includes("リ・スクリプト")),
		true,
	);
	assert.equal(
		utteranceTexts.some((text) => text.includes("リスクリプト")),
...
test("build-project resolves character voices per utterance speaker_key", async () => {
	...
	for (const utterance of textJson.utterances) {
		...
		assert.equal(
			...
			expectedStyleByCharacterKey[characterKey],
		);
	}
});
...
test("build-project rejects unknown character_key in utterance", async () => {
	...
	await writeFile(
		scriptPath,
		["1. 導入", "[speaker:ghost] この話者キーは未定義です。"].join("\n"),
		"utf-8",
	);
...
test("build-project normalizes too-old appVersion to supported vvproj format version", async () => {
	...
	const oldSynthesisDefaultsPath = path.join(
		tempRoot,
		"old-synthesis-defaults.json",
	);
	await writeFile(
	...
	await withMockVoicevoxServer(
		...
		async (voicevoxApiUrl) => {
			...
			const textUtteranceById = new Map(
				textJson.utterances.map(
					(utterance) => [utterance.utterance_id, utterance] as const,
				),
			);
			...
			for (const audioKey of projectJson.talk.audioKeys) {
				...
				assert.equal(
					...
					sourceUtterance.pause_length_ms / 1000,
				);
			}
		},
	);
...
test("build-text infers run-dir from --script path when run-dir is omitted", async () => {
	...
	assert.equal(
		path.dirname(buildTextResult.voicevoxTextJsonPath),
		path.join(runDir, "voicevox_text"),
	);
	const textJson = JSON.parse(
		await readFile(buildTextResult.voicevoxTextJsonPath, "utf-8"),
	) as VoicevoxTextJsonTest;
...
test("build-text stores source_script_path as run-dir relative path", async () => {
	...
	await writeFile(
		scriptPath,
		["1. 導入", "これは source_script_path の検証です。"].join("\n"),
		"utf-8",
	);
	...
	assert.notEqual(
		textJson.meta.source_script_path,
...
test("build-project infers run-dir from --voicevox-text-json path when run-dir is omitted", async () => {
	...
	assert.equal(
		path.dirname(projectResult.importJsonPath),
		path.join(runDir, "voicevox_project"),
	);
});
...
test("build-text falls back to built-in defaults when build-text config path is omitted", async () => {
	const tempRoot = await mkdtemp(path.join(os.tmpdir(), "narrative-vox-test-"));
	const withoutConfigRunDir = path.join(
		tempRoot,
		"introducing-rescript",
		"run-20260211-7778",
	);
	const explicitConfigRunDir = path.join(
		tempRoot,
		"introducing-rescript",
		"run-20260211-7779",
	);
	...
	const builtInConfigPath = path.join(
		tempRoot,
		"built-in-build-text-config.json",
	);
	await writeFile(
		builtInConfigPath,
...
test("build-text extracts dictionary candidates with readings from morphological analysis", async () => {
	...
	await writeFile(
		scriptPath,
		["1. 導入", "検証の流れを整理する。", "APIの挙動も検証する。"].join("\n"),
		"utf-8",
	);
	...
	const kensho = dictionary.find(
		(item: { surface: string; reading_or_empty: string }) =>
			item.surface === "検証",
	);
	assert.ok(kensho);
	assert.equal(kensho.reading_or_empty.length > 0, true);

	const api = dictionary.find(
		(item: { surface: string; reading_or_empty: string }) =>
			item.surface === "API",
	);
	assert.ok(api);
...
test("build-text adds warning when speakability score is low", async () => {
	...
	assert.equal(
		textJson.quality_checks.warnings.some((message) =>
			message.includes("Speakability score is low"),
		),
		true,
	);
});
...
test("build-text applies build-text config values to pause and warning thresholds", async () => {
	...
	await writeFile(scriptPath, ["1. 導入", "終わり。"].join("\n"), "utf-8");
	...
	const baseJson = JSON.parse(
		await readFile(baseResult.voicevoxTextJsonPath, "utf-8"),
	) as VoicevoxTextJsonTest;
	assert.equal(baseJson.utterances[0]?.pause_length_ms, 320);
	assert.equal(
		baseJson.quality_checks.warnings.some((message) =>
			message.includes("Speakability score is low"),
		),
		false,
	);
	...
	const customJson = JSON.parse(
		await readFile(customResult.voicevoxTextJsonPath, "utf-8"),
	) as VoicevoxTextJsonTest;
	assert.equal(customJson.utterances[0]?.pause_length_ms, 410);
	assert.equal(
		customJson.quality_checks.warnings.some((message) =>
			message.includes("Speakability score is low"),
		),
		true,
	);
