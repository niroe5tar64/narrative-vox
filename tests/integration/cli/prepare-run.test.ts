...
test("findLatestRunDir chooses newest run id", async () => {
	const tempRoot = await mkdtemp(
		path.join(os.tmpdir(), "narrative-vox-new-run-"),
	);
...
test("cloneRunDirectories copies blueprint/material/script into target run", async () => {
	...
	await writeFile(
		path.join(sourceRunDir, "blueprint", "project_blueprint.json"),
		'{"ok":true}\n',
		"utf-8",
	);
	await writeFile(
		path.join(sourceRunDir, "material", "E01_material.json"),
		'{"ok":true}\n',
		"utf-8",
	);
	await writeFile(
		path.join(sourceRunDir, "script", "E01_script.md"),
		"1. 見出し\n本文\n",
		"utf-8",
	);
	...
	const blueprint = await readFile(
		path.join(runDir, "blueprint", "project_blueprint.json"),
		"utf-8",
	);
	const material = await readFile(
		path.join(runDir, "material", "E01_material.json"),
		"utf-8",
	);
	const script = await readFile(
		path.join(runDir, "script", "E01_script.md"),
		"utf-8",
	);
