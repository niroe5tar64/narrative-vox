...
describe("resolvePromptTemplatePath", () => {
  ...
  test("returns correct path for material step", () => {
    ...
    assert.equal(
      result,
      path.resolve("prompts/tech-explainer/episode-material.md"),
    );
  });

  test("throws on unknown step", () => {
    assert.throws(
      () => resolvePromptTemplatePath("tech_explainer", "unknown"),
      /Unknown step/,
    );
  });
});
...
describe("resolvePromptTemplate", () => {
  ...
  test("reports unresolved placeholders", () => {
    const template = ["## Prompt", "", "{{KNOWN}} and {{UNKNOWN_KEY}}"].join(
      "\n",
    );
...
describe("renderPrompt", () => {
  test("resolves blueprint template with project config", async () => {
    ...
    assert.ok(
      result.resolvedPrompt.includes("Tech Explainer: Example Tech Explainer"),
    );
    assert.ok(
      result.templatePath.endsWith("prompts/tech-explainer/blueprint.md"),
    );
    assert.deepEqual(result.unresolvedKeys, []);
  });
  ...
  test("resolves material template with project config and episodeId override", async () => {
    ...
    assert.ok(
      !result.resolvedPrompt.includes("- 対象エピソードID: `{{EPISODE_ID}}`"),
    );
  ...
  test("throws on unresolved placeholder", async () => {
    ...
    const tmpConfig = path.resolve(
      "configs/pipeline/projects/_test_incomplete.json",
    );
    await writeFile(
      tmpConfig,
      JSON.stringify({ GENRE_ID: "tech-explainer", PROJECT_TITLE: "Test" }),
    );
    try {
