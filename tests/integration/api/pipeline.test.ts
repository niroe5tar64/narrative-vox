...
afterAll(async () => {
  ...
  try {
    ...
    if (jobsRes.status === 202) {
      ...
      await fetch(`${baseUrl}/api/pipeline/${jobId}/cancel`, {
        method: "POST",
      });
    }
  } catch {
...
test("WS: 存在しないジョブに接続すると 1008 でクローズされる", async () => {
  ...
  assert.equal(
    closeCode,
    1008,
    "Job not found は 1008 Policy Violation でクローズ",
  );
...
test("WS: cancel されたジョブの終了イベントには cancelled=true が含まれる", async () => {
  ...
  if (cancelBody.cancelled === true) {
    ...
    assert.ok(
      exitEvent !== undefined,
      "Process exited イベントが届いていること",
    );
    assert.equal(
      exitEvent.cancelled,
      true,
      "cancelled イベントには cancelled=true が含まれる",
    );
  } else {
