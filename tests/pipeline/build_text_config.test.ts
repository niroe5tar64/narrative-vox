...
} from "../../src/pipeline/build_text/build_text_config.ts";
...
async function writeConfig(dir: string, data: unknown): Promise<string> {
  const filePath = path.join(dir, "build_text_config.json");
...
test("loadBuildTextConfig loads valid config file", async () => {
  ...
  assert.equal(config.speakability.warningThresholds.minTerminalPunctuationRatio, 0.65);
