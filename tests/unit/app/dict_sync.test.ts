...
import { syncUserDict } from "../../../src/app/dict_sync.ts";
...
test("syncUserDict: adds words from dict file to empty engine", async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({
      version: 1,
      words: [
        { surface: "TypeScript", pronunciation: "タイプスクリプト", accent_type: 5 },
  ...
  try {
    ...
    assert.equal(firstPostUrl.searchParams.get("pronunciation"), "タイプスクリプト");
...
test("syncUserDict: deletes existing entries before adding", async () => {
  ...
  const existingDict = {
    "uuid-1": { surface: "OldWord", pronunciation: "オールドワード", accent_type: 0, word_type: "PROPER_NOUN", priority: 5, mora_count: 6 },
    "uuid-2": { surface: "Another", pronunciation: "アナザー", accent_type: 0, word_type: "PROPER_NOUN", priority: 5, mora_count: 3 }
  };
...
test("syncUserDict: empty words array syncs correctly", async () => {
  ...
  await writeFile(
    dictPath,
    JSON.stringify({ version: 1, words: [] })
  );

  const existingDict = {
    "uuid-x": { surface: "X", pronunciation: "エックス", accent_type: 0, word_type: "PROPER_NOUN", priority: 5, mora_count: 4 }
  };
