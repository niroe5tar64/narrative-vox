...
function coerceOutputSamplingRate(
  value: number | string | undefined,
...
export function normalizeSynthesisDefaults(
  raw: RawSynthesisDefaults,
): SynthesisDefaults {
  const normalizedQueryDefaults: SynthesisQueryDefaults = {
    speedScale: requireFiniteNumber(
      raw.queryDefaults?.speedScale,
      "queryDefaults.speedScale",
    ),
    pitchScale: requireFiniteNumber(
      raw.queryDefaults?.pitchScale,
      "queryDefaults.pitchScale",
    ),
    intonationScale: requireFiniteNumber(
      raw.queryDefaults?.intonationScale,
      "queryDefaults.intonationScale",
    ),
    volumeScale: requireFiniteNumber(
      raw.queryDefaults?.volumeScale,
      "queryDefaults.volumeScale",
    ),
    ...
    outputSamplingRate: coerceOutputSamplingRate(
      raw.queryDefaults?.outputSamplingRate,
    ),
    outputStereo:
      raw.queryDefaults?.outputStereo ?? DEFAULT_QUERY_DEFAULTS.outputStereo,
  };
  ...
  return {
    ...
    timeSignature: {
      beats: requireFiniteNumber(
        raw.timeSignature?.beats,
        "timeSignature.beats",
      ),
      beatType: requireFiniteNumber(
        raw.timeSignature?.beatType,
