...
async function isVoicevoxApiReachable(
  url: string,
  timeoutMs = 2000,
): Promise<boolean> {
...
function normalizeAccentPhrase(
  raw: VoicevoxQueryAccentPhraseLike,
): VoicevoxQueryAccentPhrase {
  const pauseMora = raw.pauseMora ?? raw.pause_mora;
  const mapped: VoicevoxQueryAccentPhrase = {
    moras: Array.isArray(raw.moras)
      ? raw.moras.map((mora) =>
          normalizeMora((mora ?? {}) as VoicevoxQueryMoraLike),
        )
      : [],
...
function normalizeAudioQueryResponse(
  raw: VoicevoxQueryLike,
): VoicevoxAudioQuery {
  ...
  const accentPhrases = Array.isArray(accentSource)
    ? accentSource.map((phrase) =>
        normalizeAccentPhrase((phrase ?? {}) as VoicevoxQueryAccentPhraseLike),
      )
    : [];
  ...
  return {
    ...
    intonationScale:
      typeof raw.intonationScale === "number" ? raw.intonationScale : 1,
    volumeScale: typeof raw.volumeScale === "number" ? raw.volumeScale : 1,
    pauseLengthScale:
      typeof raw.pauseLengthScale === "number" ? raw.pauseLengthScale : 1,
    prePhonemeLength:
      typeof raw.prePhonemeLength === "number" ? raw.prePhonemeLength : 0.1,
    postPhonemeLength:
      typeof raw.postPhonemeLength === "number" ? raw.postPhonemeLength : 0.1,
    outputSamplingRate:
      typeof raw.outputSamplingRate === "number" ||
      raw.outputSamplingRate === "engineDefault"
        ? raw.outputSamplingRate
        : "engineDefault",
    outputStereo:
      typeof raw.outputStereo === "boolean" ? raw.outputStereo : false,
...
async function requestWithRetry({
  ...
  const mergedConfig: VoicevoxRequestRetryConfig = {
    maxAttempts:
      retryConfig?.maxAttempts ?? DEFAULT_VOICEVOX_RETRY_CONFIG.maxAttempts,
    baseDelayMs:
      retryConfig?.baseDelayMs ?? DEFAULT_VOICEVOX_RETRY_CONFIG.baseDelayMs,
    timeoutMs:
      retryConfig?.timeoutMs ?? DEFAULT_VOICEVOX_RETRY_CONFIG.timeoutMs,
  };
  ...
  while (attempts < mergedConfig.maxAttempts) {
    ...
    if (!response.ok) {
      ...
      const retriable =
        isRetriableStatus(statusCode) && attempts < mergedConfig.maxAttempts;
      if (retriable) {
        await wait(backoffMs(attempts, mergedConfig.baseDelayMs));
        continue;
      }
...
export async function fetchAudioQueryFromEngine({
  ...
  const endpoint = new URL(
    "/audio_query",
    normalizeVoicevoxApiUrl(voicevoxApiUrl),
  );
