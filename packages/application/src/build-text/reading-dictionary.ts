...
export async function loadReadingDictionary(
  filePath: string,
): Promise<ReadingDictionary> {
  const resolvedPath = path.resolve(filePath);
  try {
    return await loadJson<ReadingDictionary>(
      resolvedPath,
      SchemaPaths.readingDictionary,
    );
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(
      `Failed to load reading dictionary (${resolvedPath}): ${message}`,
    );
  }
}

export function applyReadingDictionary(
  text: string,
