...
import type { MorphTokenizer } from "./dictionary-tokenizer.ts";

export {
  getJapaneseMorphTokenizer,
  type MorphTokenizer,
} from "./dictionary-tokenizer.ts";
...
export function inferReadingFromSurface(surface: string): string {
  ...
  return [...surface]
    .map(
      (char) =>
        UPPERCASE_ASCII_READING_MAP[
          char as keyof typeof UPPERCASE_ASCII_READING_MAP
        ] || "",
    )
    .join("");
}
...
export function collectRubyCandidates(
  text: string,
  map: TermCandidateMap,
): void {
  const RUBY_RE = /\{([^|{}]+)\|([^{}]+)\}/g;
...
export function collectTermCandidatesWithMorphology(
  ...
  for (const token of tokenizer.tokenize(text)) {
    ...
    const inferredReading = readingFromMorph
      ? ""
      : inferReadingFromSurface(surface);
    const reading = readingFromMorph || inferredReading;
    upsertTermCandidate(map, {
      surface,
      reading,
      source: "morph",
      readingSource: readingFromMorph
        ? "morph"
        : inferredReading
          ? "inferred"
          : "",
...
export function priorityForCandidate(
  candidate: TermCandidateState,
): CandidatePriority {
  ...
  if (
    ...
    (hasMorphReading || candidate.source === "morph")
  ) {
    return "HIGH";
  }
...
export function toDictionaryCandidates(
  termCandidates: TermCandidateMap,
): DictionaryCandidate[] {
