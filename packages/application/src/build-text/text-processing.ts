...
function collectPreferredSplitPoints(text: string): number[] {
  ...
  return [...points]
    .filter(
      (point) =>
        point >= MIN_SPLITTABLE_CHARS &&
        text.length - point >= MIN_SPLITTABLE_CHARS,
    )
    .sort((a, b) => a - b);
}
...
function chooseSplitPoint(text: string, maxCharsPerSentence: number): number {
  ...
  const pointsWithinLimit = preferredPoints.filter(
    (point) => point <= maxCharsPerSentence,
  );
  ...
  const nearestAfterLimit = preferredPoints.find(
    (point) =>
      point > maxCharsPerSentence &&
      point <= maxCharsPerSentence + SPLIT_POINT_TOLERANCE,
  );
...
export function decidePauseLengthMs(
  ...
  const lengthBonus = clampNumber(
    Math.floor((length - 18) / pauseConfig.lengthBonus.step) *
      pauseConfig.lengthBonus.increment,
    0,
  ...
  const conjunctionPenalty = CONJUNCTION_PLAIN_RE.test(trimmed)
    ...
    : 0;
  const continuationPenalty = isTerminalInSourceLine
    ? 0
    : pauseConfig.penalties.continuation;

  const rawPause =
    base + lengthBonus - conjunctionPenalty - continuationPenalty;
...
export function evaluateSpeakability(
  ...
  const longCount = lengths.filter(
    (length) => length > DEFAULT_MAX_CHARS_PER_SENTENCE,
  ).length;
  ...
  const punctuationPenalty =
    (1 - terminalRatio) * scoringConfig.punctuationWeight;
  const score = Math.round(
    clampNumber(100 - avgPenalty - longPenalty - punctuationPenalty, 0, 100),
  );
