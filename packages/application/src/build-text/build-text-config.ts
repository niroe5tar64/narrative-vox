...
export function normalizeBuildTextConfig(
  raw?: RawBuildTextConfig,
): BuildTextConfig {
  if (!raw) {
    return {
      speakability: {
        warningThresholds: {
          ...DEFAULT_BUILD_TEXT_CONFIG.speakability.warningThresholds,
        },
  ...
  const result: BuildTextConfig = {
    speakability: {
      warningThresholds: {
        scoreThreshold: requireFiniteNumber(
          warningThresholds?.scoreThreshold,
          "speakability.warningThresholds.scoreThreshold",
        ),
      ...
      scoring: {
        targetAverageChars: requireFiniteNumber(
          scoring?.targetAverageChars,
          "speakability.scoring.targetAverageChars",
        ),
        ...
        longRatioWeight: requireFiniteNumber(
          scoring?.longRatioWeight,
          "speakability.scoring.longRatioWeight",
        ),
    ...
    pause: {
      ...
      bases: {
        ...
        strongEnding: requireFiniteNumber(
          pause?.bases?.strongEnding,
          "pause.bases.strongEnding",
        ),
        fullStop: requireFiniteNumber(
          pause?.bases?.fullStop,
          "pause.bases.fullStop",
        ),
        clauseEnd: requireFiniteNumber(
          pause?.bases?.clauseEnd,
          "pause.bases.clauseEnd",
        ),
      },
      lengthBonus: {
        step: requireFiniteNumber(
          pause?.lengthBonus?.step,
          "pause.lengthBonus.step",
        ),
        increment: requireFiniteNumber(
          pause?.lengthBonus?.increment,
          "pause.lengthBonus.increment",
        ),
        max: requireFiniteNumber(
          pause?.lengthBonus?.max,
          "pause.lengthBonus.max",
        ),
      },
      penalties: {
        conjunction: requireFiniteNumber(
          pause?.penalties?.conjunction,
          "pause.penalties.conjunction",
        ),
        continuation: requireFiniteNumber(
...
export async function loadBuildTextConfig(
  configPath: string,
): Promise<BuildTextConfig> {
  const resolvedPath = path.resolve(configPath);
  try {
    const raw = await loadJson<RawBuildTextConfig>(
      resolvedPath,
      SchemaPaths.buildTextConfig,
    );
    return normalizeBuildTextConfig(raw);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(
      `Failed to load build-text config (${resolvedPath}): ${message}`,
    );
  }
}
