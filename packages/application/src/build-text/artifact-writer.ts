...
> = {
  ...
  [DictionaryCsvField.occurrences]: (candidate) =>
...
export async function writeBuildTextArtifacts(
  paths: BuildTextArtifactPaths,
  voicevoxTextData: VoicevoxTextData,
): Promise<void> {
  await mkdir(paths.voicevoxTextDir, { recursive: true });
  await mkdir(paths.dictionaryDir, { recursive: true });

  await writeFile(
    paths.voicevoxTextJsonPath,
    `${JSON.stringify(voicevoxTextData, null, 2)}\n`,
    "utf-8",
  );
  await writeFile(
