...
export function resolveBuildTextOutputPaths({
  ...
  const inferredRunDir = runDir
    ? path.resolve(runDir)
    : inferRunDirFromScriptPath(resolvedScriptPath);
  if (!inferredRunDir) {
    throw new Error(
