...
async function ensureVoicevoxReachable(
  voicevoxApiUrl: string,
  timeoutMs = 2000,
): Promise<void> {
...
export async function validateBuildPrerequisites(
  ...
  try {
    resolvedSynthesisDefaultsPath = await resolveSynthesisDefaultsPath(
      options.synthesisDefaultsPath,
    );
  ...
  if (requiresCharacterVoice) {
    try {
      ...
      if (
        ...
        !characterMap.characters[options.characterKey]
      ) {
        throw new Error(
      ...
      if (options.emotion) {
        const targetCharacterKeys = options.characterKey
          ? [options.characterKey]
          : speakerKeys;
        for (const characterKey of targetCharacterKeys) {
          ...
          if (!emotionMap) {
            throw new Error(
              `Character "${characterKey}" has no emotionStyles defined`,
            );
          }
  ...
  if (options.speedPreset) {
    try {
      resolvedSpeedProfilesPath = await resolveSpeedProfilesPath(
        options.speedProfilesPath,
      );
      if (!resolvedSpeedProfilesPath) {
        throw new Error(
          `Speed preset "${options.speedPreset}" was requested but no speed profiles are configured. Set --speed-profiles or add configs/voice/voicevox/speed-profiles.json`,
        );
      }
  ...
  try {
    resolvedVoicevoxApiUrl = await resolveVoicevoxApiUrl(
