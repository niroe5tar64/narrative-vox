...
| `build-project` | `--voicevox-text-json voicevox_text/E##_voicevox_text.json` | `--run-dir` を `.../voicevox_text/...` から推論。`--synthesis-defaults` 未指定時は `synthesis-defaults.json` を使用（未作成ならエラー）。 | `voicevox_project/*_voicevox_import.json`, `voicevox_project/*.vvproj`, `voicevox_project/*_project_meta.json` |
