...
function buildProjectOptions(options: CliOptions) {
  return {
    ...
    intonationScaleDelta: optionAsNumber(options, "intonation-scale-delta")
  };
}
...
const commandHandlers: Record<CommandName, CommandHandler> = {
  ...
  "build-project": async (options) => {
    ...
    console.log(`- ${path.relative(process.cwd(), result.projectMetaJsonPath)}`);
  },
