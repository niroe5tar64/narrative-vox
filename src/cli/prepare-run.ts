...
import { pathExists } from "../infra/fs-utils.ts";
import { RUN_ID_RE, makeRunIdNow, validateRunId } from "../domain/run-id.ts";

export { makeRunIdNow, validateRunId };

const STAGES_TO_COPY = ["blueprint", "material", "script"] as const;
...
function printUsage() {
	console.log(`Usage:
  bun src/cli/prepare_run.ts [--run-dir <projects/.../run-YYYYMMDD-HHMM>] [--source-run-dir <projects/.../run-YYYYMMDD-HHMM>] [--project-id <id>] [--run-id <run-YYYYMMDD-HHMM>] [--projects-dir <projects>] [--default-project-id <id>] [--default-source-run-dir <projects/.../run-YYYYMMDD-HHMM>] [--default-run-id <run-YYYYMMDD-HHMM>] [--no-prompt]

Behavior:
  - Missing arguments are asked interactively when TTY is available.
  - Default values can be overridden by --default-* options.
  - Copies blueprint/material/script from source run into target run directory.
`);
}
...
export async function runPrepareRun(options: CliOptions) {
	const showHelp = Boolean(options.help);
	if (showHelp) {
		printUsage();
		return;
	}
