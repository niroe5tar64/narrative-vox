...
import { parseSpeakerTag } from "../domain/speaker-tag.ts";
import { normalizeSynthesisDefaults, type RawSynthesisDefaults } from "../domain/synthesis-defaults.ts";
import { resolveCharacterMap } from "../infra/character-map-resolver.ts";
import { resolveSynthesisDefaultsPath, resolveSpeedProfilesPath } from "../infra/config-resolver.ts";
import { resolveVoicevoxApiUrl } from "../infra/voicevox-engine.ts";
import { loadSpeedProfiles } from "../app/build-project/speed-profiles.ts";
  ...
  if (options.speedPreset) {
    try {
      ...
      if (!resolvedSpeedProfilesPath) {
        throw new Error(
          `Speed preset "${options.speedPreset}" was requested but no speed profiles are configured. Set --speed-profiles or add configs/voicevox/speed-profiles.json`
        );
      }
