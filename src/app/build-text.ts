...
import { parseSectionHeader } from "../domain/script-structure.ts";
import { SchemaPaths } from "../infra/schema-paths.ts";
import { hasSpeakerTagPrefix, parseSpeakerTag } from "../domain/speaker-tag.ts";
import type { VoicevoxTextData, VoicevoxTextQualityChecks, VoicevoxTextUtterance } from "../domain/types.ts";
import {
  splitIntoSentences,
  decidePauseLengthMs,
...
import { writeBuildTextArtifacts, BuildTextArtifactPaths } from "./build-text/artifact-writer.ts";
...
export { applyReadingDictionary, loadReadingDictionary } from "./build-text/reading-dictionary.ts";
...
function buildUtterancesAndCandidates(
  ...
  for (const rawLine of lines) {
    ...
    const withDictionary = applyReadingDictionary(withoutRuby, readingDictionary);
...
export async function buildText({
  ...
  const resolvedReadingDictionaryPath = path.resolve(
    readingDictionaryPath ?? "configs/voicevox/reading-dictionary.json"
  );
  const readingDictionary = await loadReadingDictionary(resolvedReadingDictionaryPath);
