...
import { safeResolve, SafePathError } from "../lib/safe-path.ts";
...
async function readJsonFile(relPath: string): Promise<{ data: unknown; absPath: string } | null> {
...
async function parseBody(c: Parameters<typeof problem>[0]): Promise<{ ok: true; body: unknown } | { ok: false; res: Response }> {
  try {
    return { ok: true, body: await c.req.json() };
  } catch {
    return { ok: false, res: problem(c, { title: "Invalid JSON body", status: STATUS_400 }) };
  }
}

async function runValidation(c: Parameters<typeof problem>[0], data: unknown, schemaName: string): Promise<{ ok: true } | { ok: false; res: Response }> {
  let errors: string[] | null;
  try {
    errors = await validateConfig(data, schemaName);
  } catch {
    return { ok: false, res: problem(c, { title: "Internal validation error", status: STATUS_500 }) };
  }
  if (errors) {
    return { ok: false, res: problem(c, { title: "Validation failed", status: STATUS_422, details: errors }) };
  }
  return { ok: true };
}
...
configsRouter.get("/characters", async (c) => {
  try {
    ...
    return problem(c, { title: "Failed to list characters", status: STATUS_500 });
  }
});
...
configsRouter.get("/characters/:key", async (c) => {
  ...
  if (!isValidKey(key)) {
    return problem(c, { title: "Invalid key", status: STATUS_400, detail: "Key must match [a-z0-9][a-z0-9_-]*" });
  }
  try {
    const result = await readJsonFile(`configs/content/characters/${key}.json`);
    if (!result) return problem(c, { title: "Character not found", status: STATUS_404 });
    return c.json(result.data);
  } catch (e) {
    if (e instanceof SafePathError) return problem(c, { title: "Forbidden", status: STATUS_403 });
    return problem(c, { title: "Failed to read character", status: STATUS_500 });
  }
});
...
configsRouter.post("/characters", async (c) => {
  ...
  try {
    ...
    if (existing) {
      return problem(c, { title: "Character already exists", status: STATUS_409, detail: `Character "${key}" already exists. Use PUT to update.` });
    }
    await writeJsonFile(relPath, parsed.body);
    return c.json(parsed.body, 201);
  } catch (e) {
    if (e instanceof SafePathError) return problem(c, { title: "Forbidden", status: STATUS_403 });
    return problem(c, { title: "Failed to create character", status: STATUS_500 });
  }
});
...
configsRouter.put("/characters/:key", async (c) => {
  ...
  if (bodyKey && bodyKey !== key) {
    return problem(c, { title: "Key mismatch", status: STATUS_400, detail: "Body 'key' must match URL parameter" });
  }
  ...
  try {
    const relPath = `configs/content/characters/${key}.json`;
    const existing = await readJsonFile(relPath);
    if (!existing) return problem(c, { title: "Character not found", status: STATUS_404 });
    await writeJsonFile(relPath, parsed.body);
    return c.json(parsed.body);
  } catch (e) {
    if (e instanceof SafePathError) return problem(c, { title: "Forbidden", status: STATUS_403 });
    return problem(c, { title: "Failed to update character", status: STATUS_500 });
  }
});
...
configsRouter.delete("/characters/:key", async (c) => {
  ...
  try {
    ...
    if (e instanceof SafePathError) return problem(c, { title: "Forbidden", status: STATUS_403 });
    return problem(c, { title: "Failed to delete character", status: STATUS_500 });
  }
});
...
configsRouter.get("/projects/:id", async (c) => {
  ...
  if (!isValidKey(id)) {
    return problem(c, { title: "Invalid project id", status: STATUS_400, detail: "ID must match [a-z0-9][a-z0-9_-]*" });
  }
  try {
    const result = await readJsonFile(`configs/pipeline/projects/${id}.json`);
    if (!result) return problem(c, { title: "Project not found", status: STATUS_404 });
    return c.json(result.data);
  } catch (e) {
    if (e instanceof SafePathError) return problem(c, { title: "Forbidden", status: STATUS_403 });
...
configsRouter.post("/projects", async (c) => {
  ...
  if (!isValidKey(id)) {
    return problem(c, { title: "Invalid PROJECT_ID", status: STATUS_422, detail: "PROJECT_ID must match [a-z0-9][a-z0-9_-]*" });
  }
  ...
  try {
    ...
    const existing = await readJsonFile(relPath);
    if (existing) {
      return problem(c, { title: "Project already exists", status: STATUS_409, detail: `Project "${id}" already exists. Use PUT to update.` });
    }
    await writeJsonFile(relPath, parsed.body);
    return c.json(parsed.body, 201);
  } catch (e) {
    if (e instanceof SafePathError) return problem(c, { title: "Forbidden", status: STATUS_403 });
    return problem(c, { title: "Failed to create project", status: STATUS_500 });
  }
});
...
configsRouter.put("/projects/:id", async (c) => {
  ...
  if (bodyId && bodyId !== id) {
    return problem(c, { title: "ID mismatch", status: STATUS_400, detail: "Body 'PROJECT_ID' must match URL parameter" });
  }
  ...
  try {
    const relPath = `configs/pipeline/projects/${id}.json`;
    const existing = await readJsonFile(relPath);
    if (!existing) return problem(c, { title: "Project not found", status: STATUS_404 });
    await writeJsonFile(relPath, parsed.body);
    return c.json(parsed.body);
  } catch (e) {
    if (e instanceof SafePathError) return problem(c, { title: "Forbidden", status: STATUS_403 });
    return problem(c, { title: "Failed to update project", status: STATUS_500 });
  }
});
...
configsRouter.delete("/projects/:id", async (c) => {
  ...
  try {
    ...
    if (e instanceof SafePathError) return problem(c, { title: "Forbidden", status: STATUS_403 });
    return problem(c, { title: "Failed to delete project", status: STATUS_500 });
  }
});
...
configsRouter.get("/styles/:id", async (c) => {
  ...
  try {
    ...
    if (!result) return problem(c, { title: "Style not found", status: STATUS_404 });
    return c.json(result.data);
  } catch (e) {
    if (e instanceof SafePathError) return problem(c, { title: "Forbidden", status: STATUS_403 });
...
for (const [name, entry] of Object.entries(VOICEVOX_CONFIGS)) {
  configsRouter.get(`/voice/voicevox/${name}`, async (c) => {
    try {
      ...
      if (!result) return problem(c, { title: `${name} not found`, status: STATUS_404 });
      return c.json(result.data);
    } catch (e) {
      if (e instanceof SafePathError) return problem(c, { title: "Forbidden", status: STATUS_403 });
      return problem(c, { title: `Failed to read ${name}`, status: STATUS_500 });
    }
  });
  ...
  configsRouter.put(`/voice/voicevox/${name}`, async (c) => {
    ...
    try {
      ...
      if (e instanceof SafePathError) return problem(c, { title: "Forbidden", status: STATUS_403 });
      return problem(c, { title: `Failed to update ${name}`, status: STATUS_500 });
    }
  });
}
