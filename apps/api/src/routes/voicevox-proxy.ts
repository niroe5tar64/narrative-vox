...
voicevoxProxyRouter.get("/status", async (c) => {
  try {
    ...
    return c.json({ status: "running", version: version.trim().replace(/^"|"$/g, "") });
  } catch (e) {
    ...
    return problem(c, { title: "Unexpected error checking VOICEVOX status", status: STATUS_500 });
  }
});
...
voicevoxProxyRouter.get("/speakers", async (c) => {
  try {
    ...
    return problem(c, { title: "Unexpected error fetching speakers", status: STATUS_500 });
  }
});
