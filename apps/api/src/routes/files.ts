...
runsRouter.get("/", async (c) => {
  ...
  try {
    ...
    try {
      ...
      projectIds = all.filter((p) => !p.startsWith(".") && PROJECT_ID_PATTERN.test(p));
      if (projectIdFilter) {
    ...
    type RunItem = { projectId: string; runId: string; createdAt: string; sortKey: string };
    ...
    const items = allRuns
      ...
      .map(({ projectId, runId, createdAt }) => ({ projectId, runId, createdAt }));
...
runsRouter.get("/:projectId/:runId/file", async (c) => {
  ...
  if (!filePath) {
    return problem(c, { title: "Missing path query parameter", status: STATUS_400 });
  }

  try {
    const runAbsPath = await safeResolve(`data/projects/${projectId}/${runId}`);
    const absPath = await safeResolve(`data/projects/${projectId}/${runId}/${filePath}`);
    ...
    if (!isTextExtension(fileName)) {
      return problem(c, { title: "Only text files are supported", status: STATUS_415 });
    }
...
runsRouter.put("/:projectId/:runId/file", async (c) => {
  ...
  try {
    ...
    if (!Array.isArray(updates)) {
      return problem(c, { title: "Expected utterances array in body", status: STATUS_400 });
    }
    ...
    for (const u of updates) {
      if (!u || typeof u !== "object" || typeof (u as Record<string, unknown>).utterance_id !== "string") {
        return problem(c, {
      ...
      if ("text" in rec) {
        if (typeof rec.text !== "string" || rec.text.length === 0 || rec.text.length > 200) {
          return problem(c, {
    ...
    type Utterance = { utterance_id: string; text: string; pause_length_ms: number } & Record<string, unknown>;
    const current = JSON.parse(currentContent) as { utterances: Utterance[] } & Record<string, unknown>;
    const updatedUtterances = current.utterances.map((u) => {
      ...
      return {
        ...
        ...(upd.pause_length_ms !== undefined && { pause_length_ms: upd.pause_length_ms }),
      };
    });
