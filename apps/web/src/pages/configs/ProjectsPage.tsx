      ...
      {showEditor ? (
        <div className="flex-1 overflow-y-auto rounded-xl border border-slate-200 bg-white/85 p-6 shadow-sm backdrop-blur">
          ...
          <div className="max-w-lg space-y-4">
            <FormField label="PROJECT_ID" required hint={<>英小文字・数字・ハイフンのみ推奨（例: my-project）<br />※ディレクトリ名にもなる識別子で作成後は変更不可</>}>
              <Input
            ...
            <FormField label="SOURCE_MARKDOWN_PATHS" required hint={<>glob パターンで対象 Markdown を指定（例: data/inputs/books/my-book/*.md）<br />※oss-dive ジャンルでは空欄で可</>}>
              <Input
            ...
            {showOssDiveFields && (
              <>
                <FormField label="REPO_ROOT_PATH" required hint="分析するOSSリポジトリの git clone 先ルートパスを指定（例: data/inputs/repos/my-project）。">
                  <Input
                ...
                <FormField label="DEEP_DIVE_FOCUS" required hint="OSSの何に着目して深掘りするかを指定。ブループリント・素材・台本全体の焦点になる（例: アーキテクチャと設計思想）。">
                  <Textarea
            ...
            <FormField label="BASELINE_CONTEXT_OR_EMPTY" hint="台本内で「既知」として扱う具体的な前提知識（例: TypeScript での実装パターン）。AUDIENCE_BACKGROUND が「読者の属性」なら、こちらは「説明を省ける知識」。不要なら空欄。">
              <Input
            ...
            <FormField label="EXISTING_AUDIO_SCRIPT_DIR_OR_EMPTY" hint="過去runの台本ディレクトリを引き継ぐ場合に指定（初回や引き継ぎ不要な場合は空欄）">
              <Input
            ...
            <FormField label="PROJECT_BLUEPRINT_JSON_PATH" hint="/gen-blueprint 実行後に生成された JSON のパスを指定。初回は空欄で可。">
              <Input
