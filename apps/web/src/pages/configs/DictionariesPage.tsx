...
function ReadingDictionarySection({
	...
	function addEntry() {
		setLocal(
			(l) =>
				l && { ...l, entries: [...l.entries, { surface: "", reading: "" }] },
		);
	}

	function removeEntry(i: number) {
		setLocal(
			(l) => l && { ...l, entries: l.entries.filter((_, idx) => idx !== i) },
		);
	}

	function updateEntry(
		i: number,
		patch: { surface?: string; reading?: string },
	) {
		setLocal(
			(l) =>
				l && {
					...l,
					entries: l.entries.map((e, idx) =>
						idx === i ? { ...e, ...patch } : e,
					),
				},
		);
							...
							<tr key={i} className="border-t border-slate-100">
								<td className="px-2 py-1">
									<Input
										value={entry.surface}
										onChange={(e) =>
											updateEntry(i, { surface: e.target.value })
										}
										placeholder="表記"
								...
								<td className="px-2 py-1">
									<Input
										value={entry.reading}
										onChange={(e) =>
											updateEntry(i, { reading: e.target.value })
										}
										placeholder="読み"
										className="border-0 shadow-none focus-visible:ring-0"
									/>
