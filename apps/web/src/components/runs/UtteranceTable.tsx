...
export function UtteranceTable({ data, etag, projectId, runId, filePath, onDirtyChange }: Props) {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      return api.runs.saveFile(projectId, runId, filePath, updates, currentEtag);
    },
    onSuccess: (result) => {
  ...
  const handleTextChange = (id: string, text: string) => {
    setRows((prev) =>
      prev.map((r) => (r.utterance_id === id ? { ...r, text, _modified: true } : r)),
    );
  };
  ...
  const handlePauseChange = (id: string, value: string) => {
    ...
    if (!Number.isNaN(n) && n >= 0 && n <= 2000) {
      setRows((prev) =>
        prev.map((r) =>
          r.utterance_id === id ? { ...r, pause_length_ms: n, _modified: true } : r,
        ),
      );
    }
  };
  ...
  let lastSectionId: number | undefined = undefined;
        ...
        <table className="w-full text-xs">
          <thead className="sticky top-0 bg-slate-50 border-b border-slate-200 z-10">
            <tr>
              ...
              <th className="px-3 py-2 text-left font-medium text-slate-600 w-20">話者</th>
              <th className="px-3 py-2 text-left font-medium text-slate-600">テキスト</th>
          ...
          <tbody>
            {rows.map((row, i) => (
              <Fragment key={row.utterance_id}>
                ...
                <tr className={row._modified ? "bg-amber-50" : "hover:bg-slate-50/60"}>
                  ...
                  <td className="px-3 py-1">
                    <input
                      type="text"
                      value={row.text}
                      onChange={(e) => handleTextChange(row.utterance_id, e.target.value)}
                  ...
                  <td className="px-3 py-1.5">
                    <input
                      ...
                      max={2000}
                      step={50}
                      onChange={(e) => handlePauseChange(row.utterance_id, e.target.value)}
