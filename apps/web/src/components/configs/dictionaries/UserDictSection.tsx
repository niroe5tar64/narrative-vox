...
export function UserDictSection({
  ...
  const editor = useConfigEditor<UserDict>({
    ...
    queryFn: async () => (await api.voicevox.getConfig("user-dict")) as UserDict,
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-3 py-2 text-left font-medium text-slate-600">アクセント</th>
              <th className="px-3 py-2 text-left font-medium text-slate-600">品詞</th>
          ...
          <tbody>
            {filteredWords.map((word) => {
              ...
              <tr key={i} className="border-t border-slate-100">
                ...
                <td className="px-2 py-1">
                  <Input value={word.pronunciation} onChange={(e) => updateWord(i, { pronunciation: e.target.value })} placeholder="読み（カナ）" className="w-32 border-0 shadow-none focus-visible:ring-0" />
                </td>
                <td className="px-2 py-1">
                  <div className="flex flex-col gap-0.5">
                    <Input type="number" min={0} value={word.accent_type} onChange={(e) => updateWord(i, { accent_type: Number(e.target.value) })} className="w-16 border-0 shadow-none focus-visible:ring-0" />
                    {word.pronunciation && (
                ...
                <td className="px-2 py-1">
                  <Input type="number" value={word.priority} min={0} max={10} onChange={(e) => updateWord(i, { priority: Number(e.target.value) })} className="w-16 border-0 shadow-none focus-visible:ring-0" />
                </td>
                ...
                <td className="px-2 py-1 text-center">
                  <button type="button" onClick={() => removeWord(i)} className="text-slate-400 hover:text-red-500">
